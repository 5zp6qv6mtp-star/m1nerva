...
while IFS= read -r path; do
...

  {
    ...
    echo '    if (!cloudEnabled() || !items || items.length === 0) return;'
    echo '    try {'
    ...
    echo '        };'
    echo '      });'
    echo '      await fetch(cloudUrl + "/rest/v1/links", {'
    echo '        method: "POST",'
    ...
    echo '      added = true;'
    echo '    }'
    echo '    if (!added) return;'
    echo '    saveUploads(uploads);'
    echo '    uploadForm.reset();'
    echo '    render();'
    echo '    pushUploadsToCloud(newItems).then(syncUploadsFromCloud);'
    echo '  });'
    echo '  render();'
